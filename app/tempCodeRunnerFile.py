...
def load_vto_models():
    ...
    load_ckpt(warp_model, get_ckpt(os.path.join(APP_ROOT, "checkpoints", "pf_warp_best.pt")))
    load_ckpt(gen_model, get_ckpt(os.path.join(APP_ROOT, "checkpoints", "pf_gen_best.pt")))
    warp_model.eval()
    gen_model.eval()
...
def make_power_2(img, base=16, method=Image.BICUBIC):
    try: ow, oh = img.size
    except Exception: oh, ow = img.shape
...
def transform_image_vto(img, method=Image.BICUBIC, normalize=True):
    img = make_power_2(img, float(2**4), method)
...
def process_photo(img_dir, cloth_name, pipeline=DMVTONPipeline()):
    with torch.no_grad():
        ...
        person_img = Image.fromarray(cv2.cvtColor(person_img, cv2.COLOR_BGR2RGB)).convert('RGB')
        person_img = transform_image_vto(person_img).unsqueeze(0).to(vto_device)

        cloth_img = Image.open(os.path.join(CLOTH_DIR, f"{cloth_name}.jpg")).convert('RGB')
        cloth_img = transform_image_vto(cloth_img).unsqueeze(0).to(vto_device)

        cloth_mask = Image.open(os.path.join(CLOTH_MASK_DIR, f"{cloth_name}.jpg")).convert('L')
        cloth_mask = transform_image_vto(cloth_mask, method=Image.NEAREST, normalize=False).unsqueeze(0).to(vto_device)
...
def process_video(vid_dir, cloth_name, pipeline=DMVTONPipeline()):
    with torch.no_grad():
        ...
        # Use original video's extension for the output
        ...
        # Get frame dimensions for the upscaled output
        # Assuming a 4x upscale from the 192x256 base resolution
        output_width = 192 * 4
        output_height = 256 * 4
        ...
        while True:
            ...
            person_img = Image.fromarray(cv2.cvtColor(person_frame, cv2.COLOR_BGR2RGB)).convert('RGB')
            person_img = transform_image_vto(person_img).unsqueeze(0).to(vto_device)
            ...
            # Ensure the output frame matches the writer's dimensions
