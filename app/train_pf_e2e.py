...
import val
from dataloader.viton_dataset_copy import LoadVITONDataset
...
def attention_total_variation_loss(features, mask, weight=0.1):
    """
    Encourages smoother attention maps for better detail preservation
    """
    tv_loss = 0
    for feat in features:
        # Horizontal TV
        tv_h = torch.mean(torch.abs(feat[:, :, 1:, :] - feat[:, :, :-1, :]))
        # Vertical TV
        tv_v = torch.mean(torch.abs(feat[:, :, :, 1:] - feat[:, :, :, :-1]))
        tv_loss += (tv_h + tv_v) * weight
    
    return tv_loss * mask.mean()
def get_curriculum_weights(epoch, total_epochs):
    ...
    # Start with simpler objectives, gradually add complex ones
    l1_weight = 5.0
    vgg_weight = 1.0 + 2.0 * progress  # Gradually increase perceptual loss
    skin_weight = 30.0 + 30.0 * progress  # Increase skin detail attention
    
    return l1_weight, vgg_weight, skin_weight
...
def train_batch(
    ...
    pb_warp_model, pf_warp_model, pf_gen_model = (
        models['pb_warp'],
        models['pf_warp'],
        models['pf_gen'],
    )
    ...
    person_clothes_edge = torch.FloatTensor((data['label'].cpu().numpy() == 5).astype(np.int64))+torch.FloatTensor((data['label'].cpu().numpy() == 6).astype(np.int64))+torch.FloatTensor((data['label'].cpu().numpy() == 7).astype(np.int64))
    +torch.FloatTensor((data['label'].cpu().numpy() == 3).astype(np.int64))+torch.FloatTensor((data['label'].cpu().numpy() == 8).astype(np.int64))+torch.FloatTensor((data['label'].cpu().numpy() == 10).astype(np.int64))+torch.FloatTensor((data['label'].cpu().numpy() == 11).astype(np.int64))
    ...
    ) + torch.FloatTensor((data['label'].cpu().numpy() == 2).astype(np.int64))+ torch.FloatTensor((data['label'].cpu().numpy() == 4).astype(np.int64))+ torch.FloatTensor((data['label'].cpu().numpy() == 13).astype(np.int64))
    ...
    p_tryon_un = data["p_tryon_un"].to(device)
    ...
    atv_loss = attention_total_variation_loss(warp_fea_all, person_clothes_edge.to(device))
    loss_all += 0.1 * atv_loss

    warp_optimizer.zero_grad()
    gen_optimizer.zero_grad()
    loss_all.backward()
    torch.nn.utils.clip_grad_norm_(pf_warp_model.parameters(), max_norm=5)
    torch.nn.utils.clip_grad_norm_(pf_gen_model.parameters(), max_norm=5)
    warp_optimizer.step()
    gen_optimizer.step()
...
def train_pf_e2e(opt):
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        l1_w, vgg_w, skin_w = get_curriculum_weights(epoch, epoch_num)
        ...
        if validate:
            ...
            metrics = val.run_val_pf(
                ...
                img_dir=Path(opt.valroot) / 'test' / 'image',
                save_img=False,
            )
            fid = metrics['fid']
            if fid < best_fid:
        ...
        if epoch % opt.save_period == 0:
            ...
            torch.save(gen_ckpt, weights_dir / f'pf_gen_epoch_{epoch}.pt')
            print_log(
                log_path,
