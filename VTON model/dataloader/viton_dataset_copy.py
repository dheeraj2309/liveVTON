...
class LoadVITONDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        # Person image
        img = Image.open(Path(self.dataroot) /f'{self.phase}' /f'image' / im_name).convert('RGB')
        # img = img.resize((self.width, self.height))
        img_tensor = self.transform_image(img)  # [-1,1]

        # Clothing image
        cloth = Image.open(Path(self.dataroot) /f'{self.phase}' /f'cloth' / im_name).convert('RGB')
        # cloth = cloth.resize((self.width, self.height))
        cloth_tensor = self.transform_image(cloth)  # [-1,1]


        # Clothing edge
        cloth_edge = Image.open(Path(self.dataroot) /f'{self.phase}' /f'cloth-mask' / im_name).convert('L')
        # cloth_edge = cloth_edge.resize((self.width, self.height))
        cloth_edge_tensor = self.transform_parse(cloth_edge)  # [-1,1]

        if self.phase == 'train':

            un_cloth = Image.open(Path(self.dataroot) /f'{self.phase}' /f'cloth' / c_name).convert(
                'RGB'
            )
            ...
            un_cloth_edge = Image.open(Path(self.dataroot) /f'{self.phase}' /f'cloth-mask' / c_name).convert('L')
            # un_cloth_edge = un_cloth_edge.resize((self.width, self.height))
            un_cloth_edge_tensor = self.transform_parse(un_cloth_edge)  # [-1,1]
    
            teacher_gen_output = Image.open(Path(self.dataroot) /f'{self.phase}' /f'teacher_gen_output' / f'{Path(im_name).stem}.png').convert('RGB')
            teacher_gen_output_tensor = self.transform_image(teacher_gen_output)

            # Parse map
            parse_path1 = Path(self.dataroot) / f'{self.phase}' /'image-parse-v3' /f'{Path(im_name).stem}.jpg'
            parse_path2 = Path(self.dataroot) / f'{self.phase}' /'image-parse-v3' /f'{Path(im_name).stem}.png'
            ...
            COCO_18_FROM_BODY_25 = [0, 15, 16, 17, 18, 5, 2, 6, 3, 7, 4, 12, 9, 13, 10, 14, 11, 1]
            with open(
                Path(self.dataroot) / f'{self.phase}' / 'openpose_json'/f'{Path(im_name).stem}_keypoints.json'
            ) as f:
                ...
                pose_data = pose_data.reshape((-1, 3))[COCO_18_FROM_BODY_25]
            ...
            # Densepose
            dense_data = np.load(
                Path(self.dataroot) / f'{self.phase}' /"densepose_npy" /f'{Path(im_name).stem}_densepose.npy',allow_pickle=True
            ).item()
            dense_mask = dense_data["labels"].astype(np.float32)
        ...
        if self.phase == 'train':
            return {
                ...
                'p_tryon_un':teacher_gen_output_tensor,
            }
        else:
            return {
                'image': img_tensor,
