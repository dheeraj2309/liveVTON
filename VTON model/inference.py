...
def process_vid(vid_dir,cloth_name,device,pipeline=DMVTONPipeline()):
    ...
    warp_ckpt = get_ckpt("checkpoints/dmvton_pf_warp.pt")
    load_ckpt(warp_model, warp_ckpt)
    warp_model.eval()

    gen_ckpt = get_ckpt("checkpoints/dmvton_pf_gen.pt")
    load_ckpt(gen_model, gen_ckpt)
    gen_model.eval()
    ...
    # Video writer setup
        ...
        out = cv2.VideoWriter(f"results/{vid_save[0]}_out{vid_save[1]}", fourcc, fps, (1024, 768))
...
def main():
    ...
    # Inference Pipeline
    process_photo("00311_00.jpg","00029_00",device)
    # process_type=input()
